...
while cap.isOpened():
    ...
    img, ratio, top, left = resize_img(frame)
    ...
    # predict landmarks
    face_img = ori_frame[new_bb[0][1]:new_bb[1][1], new_bb[0][0]:new_bb[1][0]]
    ...
    # visualize
    cv2.rectangle(ori_frame, pt1=tuple(ori_bb[0]), pt2=tuple(ori_bb[1]), color=(255, 255, 255), thickness=2)

    for i, l in enumerate(ori_lmks):
      cv2.putText(ori_frame, str(i), tuple(l), cv2.FONT_HERSHEY_SIMPLEX, 1, (255,255,255), 2, cv2.LINE_AA)
      cv2.circle(ori_frame, center=tuple(l), radius=1, color=(255, 255, 255), thickness=2)
    ...
    try:
      result_frame = overlay_transparent(result_frame, rotated_glasses, glasses_center[0], glasses_center[1], overlay_size=(int(glasses_size), int(glasses.shape[0] * glasses_size / glasses.shape[1])))
    except:
      print('failed overlay image')

    cv2.imshow('img', ori_frame)
    cv2.imshow('result', result_frame)
    # filename, ext = os.path.splitext(f)
    # cv2.imwrite('result/%s_lmks%s' % (filename, ext), ori_frame)
    # cv2.imwrite('result/%s_result%s' % (filename, ext), result_frame)
